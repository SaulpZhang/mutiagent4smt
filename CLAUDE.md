...
1. **基准实验（Benchmark）**: 使用不同prompt策略（zero-shot, one-shot, few-shot, chain-of-thought）让LLM直接生成SMT-LIB V2代码和Python API代码，作为性能基线
2. **系统实现**: 用Python实现上述5模块系统，确保智能体记忆隔离
3. **LLM微调**: 探索使用LoRA对领域数据进行SFT微调，对比微调前后的性能提升
4. **鲁棒性实验**: 在不同LLM上测试系统性能，评估系统的模型无关性
